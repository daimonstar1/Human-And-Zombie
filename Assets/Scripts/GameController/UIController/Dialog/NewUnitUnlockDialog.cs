...
public class NewUnitUnlockDialog : DialogController
{
    ...
    public override void OnOpen(string[] agruments = null, System.Action onCloseComplete = null)
    {
        ...
        unitTexture.mainTexture = Resources.Load<Texture2D>("Textures/Characters/Units/Unit_" + unitID.Split('-')[0] + "/Normal/Idle/1");
        unitTexture.gameObject.GetComponent<FrameAnimation>().pathToTextures = "Textures/Characters/Units/Unit_" + unitID.Split('-')[0] + "/Normal/Idle";
        unitTexture.gameObject.GetComponent<FrameAnimation>().Play();
    }
